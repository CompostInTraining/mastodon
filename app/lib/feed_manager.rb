...
class FeedManager
  ...
  def push_to_home(account, status, update: false)
    return false unless add_to_feed(:home, account.id, status, account.user&.aggregates_reblogs?, account.user&.home_dms?)
  ...
  def unpush_from_home(account, status, update: false)
    return false unless remove_from_feed(:home, account.id, status, account.user&.aggregates_reblogs?, account.user&.home_dms?)
  ...
  def push_to_list(list, status, update: false)
    return false if filter_from_list?(status, list) || !add_to_feed(:list, list.id, status, list.account.user&.aggregates_reblogs?, list.account.user&.home_dms?)
  ...
  def unpush_from_list(list, status, update: false)
    return false unless remove_from_feed(:list, list.id, status, list.account.user&.aggregates_reblogs?, list.account.user&.home_dms?)
  ...
  def merge_into_home(from_account, into_account)
    ...
    statuses.each do |status|
      next if not (status.direct_visibility? && account.user&.home_dms?) || filter_from_home?(status, into_account.id, crutches)

      add_to_feed(:home, into_account.id, status, aggregate, home_dms)
    end
  ...
  def merge_into_list(from_account, list)
    ...
    statuses.each do |status|
      ...
      add_to_feed(:list, list.id, status, aggregate, home_dms)
    end
  ...
  def unmerge_from_home(from_account, into_account)
    ...
    from_account.statuses.select('id, reblog_of_id').where(id: timeline_status_ids).reorder(nil).find_each do |status|
      remove_from_feed(:home, into_account.id, status, into_account.user&.aggregates_reblogs?, into_account.user&.home_dms?)
    end
  end
  ...
  def unmerge_from_list(from_account, list)
    ...
    from_account.statuses.select('id, reblog_of_id').where(id: timeline_status_ids).reorder(nil).find_each do |status|
      remove_from_feed(:list, list.id, status, list.account.user&.aggregates_reblogs?, list.account.user&.home_dms?)
    end
  end
  ...
  def populate_home(account)
    ...
    account.statuses.limit(limit).each do |status|
      add_to_feed(:home, account.id, status, aggregate, home_dms)
    end
