...
class MoveHomeDmsSettings < ActiveRecord::Migration[6.1]
  class User < ApplicationRecord; end

  MAPPING = {
    setting_home_dms: 'home_dms',
  }.freeze
  ...
  def up
    User.find_each do |user|
      previous_setting = LegacySetting.where(thing_type: 'User', thing_id: user.id).index_by(&:var)

      user_settings = Oj.load(user.settings || '{}')
      user_settings.delete('theme')

      MAPPING.each do |legacy_key, new_key|
        value = previous_setting[legacy_key]&.value
